...
sourceFname = 'nema1';
options = SIMIND_SPECT_parser(sourceFname);
...
fpath = 'data/nema_phantom/results/';
fname = strcat('nema_phantom_speedtest.csv');
hdr = ['source_file,nx,ny,nz,nRaySPECT,coneMethod,implementation,' ...
    'projector_type,OSEM,nIter,nSubsets,subset_type,nProjections, ntime'];
nRay = (1:12).^2;
nSamples = 100;

if ~isfile(strcat(fpath, fname))
    fID = fopen(strcat(fpath, fname), 'a');
    fprintf(fID, '%s\n', hdr);
    fclose(fID);
end
...
for nn = nRay
    for ii = 1:nSamples
        options.nRaySPECT = nn;

        tStart = tic;
        pz = reconstructions_mainSPECT(options);
        tElapsed = toc(tStart);
        disp(['Reconstruction process took ' num2str(tElapsed) ' seconds with ' num2str(nn) ' rays'])

        dataToSave = [options.Nx, options.Ny, options.Nz, ...
            options.nRaySPECT, options.coneMethod, ...
            options.implementation, options.projector_type, ...
            options.OSEM, options.Niter, options.subsets, ...
            options.subset_type, options.nProjections, tElapsed];

        fID = fopen(strcat(fpath, fname), 'a');
        fprintf(fID, '%s,', sourceFname);
        fprintf(fID, '%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%f\n', dataToSave);
        fclose(fID);

        if ii == 1
            save( ...
                strcat(fpath, sourceFname, ...
                    '_model', num2str(options.coneMethod), ...
                    '_nRay', num2str(options.nRaySPECT), '.mat' ...
                ),'pz', 'hdr', 'dataToSave', '-v7.3');
        end
    end
end