...
inline bool orthogonalHelper3D(const uint32_t tempi, const int uu, const uint32_t d_N2, const uint32_t d_N3, const uint32_t d_Nxy, const int zz, const T s2, const T l3, const T l1, const T l2,
	...
	const uint32_t nBins, const int lor, const uint16_t nRays) {
...
inline int orthDistance3D(const uint32_t tempi, const T diff1, const T diff2, const T diffZ, const T center1, const T* center2, const T* centerZ, const T temp, int temp2, const int tempk,
	...
	T* TOFCenter, const T TOFSum, const bool TOF, const uint8_t fp, const int projType, const uint32_t nBins, const int lor, const uint16_t nRays) {
	int uu = 0;
	uint32_t local_ind = 0;
	bool breikki = false;
	// y0
	...
	for (int zz = tempk; zz < maksimiZ; zz++) {
		...
		for (uu1 = temp2; uu1 < maksimiXY; uu1++) {
			breikki = orthogonalHelper3D(tempi, uu1, d_N2, d_N3, d_Nxy, zz, s2, l3, l1, l2, diff2, diffZ, kerroin, center2[uu1], bmin, bmax, Vmax, V,
				XY, ax, temp, input, Summ, output, no_norm, element, sigma_x, D, DD, TOFCenter, TOFSum, TOF, fp, projType, nBins, lor, nRays);
			if (breikki) {
				break;
			}
			uu++;
...
inline T perpendicular_elements(const uint32_t N, const T dd, const T dd2, const T b, const T d, const uint32_t N1, const uint32_t N2, const T* atten,
	...
	if (!CT) {
		T temp = 1.;
		if (nRays > 1)
			temp = dd2 / (L * static_cast<T>(nRays));
		else if (projType == 1)
			temp = dd2 / L;
...
// END SPECT EDIT
	...
	/// <param name="fp if fp == 1, forward projection is computed, if fp == 2 backprojection. Default is forward projection (optional)"></param>
...
void projectorType123Implementation4(paramStruct<T>& param, const int64_t nMeas, T* output, const T* x, const T* z,
	T* input, const bool CT = false, const bool SPECT = false, const uint8_t fp = 1, T* SensImage = nullptr, const uint16_t* detIndex = nullptr, const uint32_t nCores = 0) {
	...
	for (int64_t lo = 0LL; lo < nMeas; lo++) {
		...
		if (param.subsets > 1 && param.subsetType == 1 && param.listMode == 0) {
			...
			if ((param.listMode > 0 && param.computeSensIm == 0) || (!CT && param.listMode == 0)) {
				ix = lo;
				iy = 0;
				iz = 0;
			}
			else if (param.computeSensIm) {
		...
		if (param.useMaskFP) {
			const int64_t idx = ix + iy * (int64_t)param.size_x;
			const bool maskVal = param.maskFP[idx];
			if (maskVal == false)
				continue;
		}		

		// SPECT EDIT
		// const bool SPECT = true;
		uint32_t nShift = 1;
		...
		for (int currentShift = 0u; currentShift < nShift; currentShift++) {
			for (int lorZ = 0u; lorZ < param.nRays3D; lorZ++) {
				for (int lorXY = 0u; lorXY < param.nRays2D; lorXY++) {
					...
					if ((y_diff == 0. && x_diff == 0. && z_diff == 0.) || (y_diff == 0. && x_diff == 0.) || std::isinf(y_diff) || std::isinf(x_diff) || std::isinf(z_diff) || std::isnan(y_diff) || std::isnan(x_diff) || std::isnan(z_diff)) {
						continue;
					}
					...
					if (param.projType == 2)
						...
						kerroin = L;
						TotV = L * M_PI * param.orthWidth * param.orthWidth;
					}
					...
					if (std::fabs(z_diff) < 1e-8 && (std::fabs(y_diff) < 1e-8 || std::fabs(x_diff) < 1e-8)) {
						...
						for (uint32_t ii = 0u; ii < d_N1; ii++) {
							...
							if (param.projType > 1) {
								orthDistance3D(ii, y_diff, x_diff, z_diff, center1[ii], center2, param.z_center, temp, indO, localIndZ, detectors.xs, detectors.ys, detectors.zs, Nyx, kerroin, d_N1, d_N2, d_N3, param.Nz,
									...
									param.nBins, lor, nRays);
								if (param.TOF)
									D -= std::copysign(d_d2, DD);
							}
							else {
								if (fp == 1) {
									denominator(ax, local_ind, d_d2, input, param.TOF, d_d2, TOFSum, DD, param.TOFCenters, param.sigma_x, D, param.nBins, lor, nRays, param.projType);
								}
								else if (fp == 2) {
									rhs(temp, ax, local_ind, output, param.noSensImage, SensImage, d_d2, param.sigma_x, D, DD, param.TOFCenters, TOFSum, param.TOF, param.nBins, param.projType);
								}
							}
							local_ind += d_N3;
						}
						...
						T txu = 0., tyu = 0., tzu = 0., tc = 0., tx0 = 1e8, ty0 = 1e8, tz0 = 1e8;
						...
						if (param.attenuationCorrection && fp == 2 && param.CTAttenuation) {
							...
							for (uint32_t ii = 0u; ii < Np; ii++) {
								...
								if (tz0 < ty0 && tz0 < tx0) {
									local_ele = voxelValue(tz0, tc, L);
									tempk += uz;
									tc = tz0;
									tz0 += tzu;
								}
								else if (ty0 < tx0) {
									local_ele = voxelValue(ty0, tc, L);
									tempj += (uy);
									tc = tz0;
									ty0 += tyu;
								}
								else {
									local_ele = voxelValue(tx0, tc, L);
									tempi += ux;
									tc = tx0;
									tx0 += txu;
								}
								compute_attenuation(local_ele, local_ind, param.atten, jelppi);
								if (tempi < 0 || tempi >= d_NNx || tempj < 0 || tempj >= d_NNy || tempk < 0 || tempk >= d_NNz) {
									break;
								}
							}
							tempi = tempi_a;
						...
						for (uint32_t ii = 0u; ii < Np; ii++) {
							...
							if (param.TOF)
								TOFSum = TOFLoop(DD, local_ele, param.TOFCenters, param.sigma_x, D, param.epps, param.nBins);
							if (param.projType > 1) {
								if (ii == 0) {
									if (ux >= 0) {
										for (int kk = tempi_a - 1; kk >= 0; kk--) {
											int uu = orthDistance3D(kk, y_diff, x_diff, z_diff, center1[kk], center2, param.z_center, temp, tempj_a, tempk_a, xs, ys, detectors.zs, Nyx, kerroin, d_N1, d_N2, d_N3,
												...
												param.projType, param.nBins, lor, nRays);
											if (uu == 0)
												break;
										}
									}
